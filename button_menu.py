# button_menu v0.6.1 — DS grid + A/D counters + place-on-face
# OK opens/selects. BACK backs out. Hold BACK closes.
# Place: move A, OK saves, move D, OK saves. Hold D-pad to slide.
...
STATUS_PAGES = ("temp", "uptime", "hcount", "last", "battery")
MODE = [("AUTO", "auto"), ("MANU", "manu"), ("STOP", "stop")]
POWER = [("PWN", "restart_pwn"), ("CAP", "restart_bc"), ("BOOT", "reboot"), ("OFF", "shutdown")]
OPTS = [("Counter", "toggle"), ("Place A/D", "place"), ("Reset A/D", "reset")]
...
class MenuCanvas(Widget):
    ...
    def draw(self, canvas, drawer):
        ...
        if p.confirm:
            ...
            return
        fn = {
            "menu": self._root, "status": self._status, "handshakes": self._hs,
            "hs_detail": self._hs_detail, "mode": self._mode, "plugins": self._plugins,
            "power": self._power, "options": self._options,
    ...
    def _root(self, drawer, w, h, font, p):
        ...
        mid = "A:%s  D:%s" % (int(p.cfg.get("assoc") or 0), int(p.cfg.get("deauth") or 0))
    ...
    def _status(self, drawer, w, h, font, p):
        key = STATUS_PAGES[p.index % len(STATUS_PAGES)]
        title = {"temp": "TEMP", "uptime": "UPTIME", "hcount": "HANDSHAKES",
                 "last": "LAST HS", "battery": "BATTERY"}[key]
        drawer.text((4, 2), title, font=font, fill=view.BLACK)
        big = fonts.Bold if fonts.Bold else font
        drawer.text((4, 20), p._status_value(key), font=big, fill=view.BLACK)
...
class ButtonMenu(plugins.Plugin):
    ...
    def on_loaded(self):
        ...
        logging.info("[button_menu] v0.6.1 loaded")
    ...
    def _apply_counters(self, ui):
        ...
        try:
            ...
            if a is not None:
                ...
                a.value = str(int(self.cfg.get("assoc") or 0))
            if d is not None:
                d.xy = (dx, dy)
                d.label = ">D:" if self.placing == "deauth" else "D:"
                d.value = str(int(self.cfg.get("deauth") or 0))
        except Exception:
            pass
        if self.placing:
            try:
    ...
    def _nitems(self):
        return {
            "menu": 6, "status": len(STATUS_PAGES),
    ...
    def _poll(self):
        ...
        if self.screen == "status":
            if self._edge("LEFT"):
                self.index = (self.index - 1) % len(STATUS_PAGES)
                self._show()
